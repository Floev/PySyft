...
class View(Model):
	def __init__(self, *out_dims, id=None):
		...
		if(id is None):
			self.init("view", list(map(int,out_dims)))
		else:
			self.id = id
			self.sc = controller
			self.type = "model"
...
class Conv2d(_Conv):
	...
	def __init__(self, input_dim, output_dim, kernel, stride=1,
		...
		super(Conv2d, self).__init__(input_dim, output_dim, kernel, stride, padding, dilation, groups, bias)
...
class ConvTranspose2d(_Conv):
	r"""Applies a 2D transposed convolution/cross-correlation using a kernel.
	| :attr:`stride` shift size of filter motion.
	| :attr:`padding` size of input enlargement with zeros on both sides.
	| :attr:`dilation` inflates filter with zeros like a chess board.
	| :attr:`groups` controls the connections between inputs and outputs.
	...
	def __init__(self, input_dim, output_dim, kernel, stride=1,
		...
		super(ConvTranspose2d, self).__init__(input_dim, output_dim, kernel, 
